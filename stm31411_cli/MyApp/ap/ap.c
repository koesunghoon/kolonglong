...
static osThreadId_t      g_loadcell_task_handle = NULL;
    ...
    for (uint32_t i = 0; i < length; i += 16)
    {
      ...
      for (uint32_t j = 0; j < 16; j++)
      {
        if (i + j < length)
        {
          ...
          {
            uint8_t val = *((volatile uint8_t *)target_addr);
            cliPrintf("%c", (val >= 0x20 && val <= 0x7E) ? val : '.');
          }
          else { cliPrintf("Not valid address!!\r\n"); break; }
        }
      }
...
void apInit(void)
{
  ...
  /* LoadCell Task 생성 */
  const osThreadAttr_t loadcell_attr = {
    .name       = "LoadCellTask",
    .stack_size = 512 * 4,
    .priority   = osPriorityAboveNormal,
  };
  g_loadcell_task_handle = osThreadNew(vTaskLoadCell, NULL, &loadcell_attr);
  if (g_loadcell_task_handle == NULL)
    LOG_ERR("LoadCell Task 생성 실패");
  else
    LOG_INF("LoadCell Task 생성 완료");
}
